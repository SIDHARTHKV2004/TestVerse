...
public class TeamController {
        ...
        try {
            ...
            if (user.getRole() != UserRole.ADMIN) {
                return ResponseEntity.status(HttpStatus.FORBIDDEN)
                        .body(Map.of("error", "Only Admin can view all teams"));
            }
    ...
    public ResponseEntity<?> createTeam(@RequestBody Map<String, Object> request) {
        try {
            ...
            if (admin.getRole() != UserRole.ADMIN) {
                return ResponseEntity.status(HttpStatus.FORBIDDEN)
                        .body(Map.of("error", "Only Admin can create teams"));
            }
    ...
    public ResponseEntity<?> getMyTeams() {
        try {
            ...
            for (TeamEntity team : teams) {
                ...
                teamData.put("description",
                        team.getDescription() != null ? team.getDescription() : "");
                teamData.put("memberCount",
                        team.getMembers() != null ? team.getMembers().size() : 0);
                teamData.put("isAdmin",
                        team.getAdmin() != null
                                && team.getAdmin().getId().equals(user.getId()));
    ...
    // ===== GET ADMIN TEAM OR CREATE DEFAULT =====
    ...
    // ===== GET AVAILABLE TEAMS FOR JOIN REQUEST =====
        ...
        try {
            ...
            if (team.getMembers() != null) {
                for (UserEntity member : team.getMembers()) {
                    ...
                    if (member.getId().equals(user.getId())) {
                        return ResponseEntity.status(HttpStatus.CONFLICT)
                                .body(Map.of("error", "You are already in this team"));
                    }
                }
            }
    ...
    public ResponseEntity<?> addMember(
        ...
        try {
            ...
            boolean alreadyInTeam = team.getMembers()
                    .stream()
                    .anyMatch(m -> m.getId().equals(userId));

            if (alreadyInTeam) {
                return ResponseEntity.status(HttpStatus.CONFLICT)
                        .body(Map.of("error", "User is already in this team"));
            }
    ...
    public ResponseEntity<?> removeMember(
        ...
        try {
            ...
            return ResponseEntity.ok(
                    Map.of("message", "User removed from team successfully")
            );
    ...
    public ResponseEntity<?> deleteTeam(@PathVariable Long teamId) {
        try {
            ...
            if (admin.getRole() != UserRole.ADMIN) {
                return ResponseEntity.status(HttpStatus.FORBIDDEN)
                        .body(Map.of("error", "Only Admin can delete teams"));
            }
            ...
            return ResponseEntity.ok(
                    Map.of("message", "Team deleted successfully")
            );
